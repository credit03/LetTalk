...
public class UserModel extends BaseModel {
     ...
     * @param inListener
     */
    public void LoginByPhone(Context context, String phone, String pw, final LogInListener inListener) {

        if (TextUtils.isEmpty(phone)) {
            inListener.internalDone(new BmobException(CODE_NULL, "请填写用户名"));
            return;
        }
        if (TextUtils.isEmpty(pw)) {
            inListener.internalDone(new BmobException(CODE_NULL, "请填写密码"));
            return;
        }
        BmobUser.loginByAccount(context, phone, pw, new LogInListener<User>() {

            @Override
            public void done(User user, BmobException e) {
                // TODO Auto-generated method stub
                if (user != null) {
                    inListener.done(user, null);
                } else {
                    inListener.done(null, new BmobException(CODE_NULL, "帐号密码错误"));
                }
            }
        });
    }
     ...
     * @param listener  回调
     */
    public void register(String username, String phone, String password, String pwdagain, String shcool_tm, final LogInListener listener) {
        if (TextUtils.isEmpty(username)) {
            listener.internalDone(new BmobException(CODE_NULL, "请填写用户名"));
            return;
        }
     ...
     * @param listener
     */
    public void queryUsersByOr(String username, int limit, final FindListener<User> listener) {

        BmobUser user = BmobUser.getCurrentUser(getContext());
        BmobQuery<User> nickname = new BmobQuery();
        BmobQuery<User> query = new BmobQuery<>();

        //去掉当前用户
        try {
            query.addWhereNotEqualTo("username", user.getUsername());
            nickname.addWhereNotEqualTo("nickname", user.getUsername());
        } catch (Exception e) {
            e.printStackTrace();
        }

        nickname.addWhereContains("nickname", username);
        nickname.setLimit(limit);
        nickname.order("-createdAt");
        ...
        queries.add(nickname);
        queries.add(query);

        BmobQuery<User> mainQuery = new BmobQuery();
        mainQuery.or(queries);
        mainQuery.findObjects(getContext(), new FindListener<User>() {
            @Override
